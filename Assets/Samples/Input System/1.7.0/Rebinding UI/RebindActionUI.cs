...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        {
            InputBinding newBinding = action.bindings[bindingIndex]; // References current binding
            string oldOverridePath = newBinding.overridePath;

            action.RemoveBindingOverride(bindingIndex); // Remove the current bidning

            // Iterate through the other actions to search for dulpicates
            foreach (InputAction otherAction in action.actionMap.actions)
            {
                if (otherAction == action) {  continue; } // The otherAction is equal to the selected action to reset, so skip this action
                for (int i = 0; i < otherAction.bindings.Count; i++) // Check other bindings for dulpicates
                {
                    InputBinding binding = otherAction.bindings[i];
                    if (binding.overridePath == newBinding.path)
                    {
                        otherAction.ApplyBindingOverride(i, oldOverridePath); // Swap bindings if new path equals the override path
                    }
                }
            }
        }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            action.Disable(); // Disables the action at the beginning, preventing errors
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnCancel(
                    operation =>
                    {
                        action.Enable(); // Re-enables the action on cancellation
                ...
                .OnComplete(
                    operation =>
                    {
                        action.Enable(); // Re-enables the action on completion
                        m_RebindOverlay?.SetActive(false);
                        m_RebindStopEvent?.Invoke(this, operation);

                        // Checking for dulpicates
                        if (CheckDuplicateBindings(action, bindingIndex, allCompositeParts))
                        { // Forces the player to input a different key if they tried to duplicate bindings.
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
        ...
        private void UpdateActionLabel()
        {
            if (m_ActionLabel != null)
            {
                ...
                // Choose whether to set the action label's text from the action's name, or by a custom name set in the editor
                if (m_OverrideActionLabel) { m_ActionLabel.text = m_ActionLabelString; }
                else 
                {
                    m_ActionLabel.text = action != null ? action.name : string.Empty;
                    m_ActionLabelString = string.Empty; // Empty string when m_OverrideActionLabel is set to false, stopping the string from remembering past inputs
                }
            }
        }
